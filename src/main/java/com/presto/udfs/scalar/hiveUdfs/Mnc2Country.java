...
public class Mnc2Country {
    ...
    static {
        codeMap = JsonUtil.json2Obj(mcc2country_code_map, Map.class);
    }

    @ScalarFunction("mnc2country")
    @Description("通过mnc获取国家代号")
    @SqlNullable
    @SqlType(StandardTypes.VARCHAR)
    public static Slice mnc2country(@SqlType(StandardTypes.VARCHAR) Slice mncSlice){
        if (mncSlice == null) {
            return null;
        }
        String mnc = mncSlice.toStringUtf8().trim();
        if (mnc.length() > 3) {
            mnc = mnc.substring(0, 3);
        }
        if (codeMap.containsKey(mnc)) {
            return Slices.utf8Slice(codeMap.get(mnc));
        }
        return null;
    }
